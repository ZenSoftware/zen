...
export function GqlCaslGuard(...actions: Array<Action>) {
  ...
  class CaslGuard extends AuthGuard('jwt') {
    ...
    async canActivate(context: ExecutionContext) {
      ...
      const classSubject = this.reflector.get<string>(CASL_SUBJECT_KEY, ctx.getClass());
      const handlerSubject = this.reflector.get<string>(CASL_SUBJECT_KEY, ctx.getHandler());
